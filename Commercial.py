...
# STREAMLIT UI
# ============================================================
st.set_page_config(page_title="Commercial Comparison Tool", layout="wide")
st.title("🏨 Commercial Comparable Matcher Tool Final")
...
if uploaded_file:
    ...
    for col in ['Market Value-2024', 'VPU']:
        df[col] = pd.to_numeric(df[col], errors='coerce')

    df = df.dropna(subset=['Market Value-2024', 'VPU'])
    ...
    if st.button("🚀 Run Matching"):
        ...
        with st.spinner("🔍 Matching Commercial, please wait..."):
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                match_columns = [
                    'Property Address', 'State', 'Property County', 'Property Account No',
                    'Owner Name/ LLC Name', 'GBA', 'Market Value-2024',
                    'VPU'
                ]
                ...
                # ----------- FIXED HEADER BLOCK ---------------
                result_start_col = status_col + 2
                col = result_start_col  # IMPORTANT

                for r in range(1, max_results_per_row + 1):
                    for colname in all_columns:
                        worksheet.write(row, col, f"Result{r}_{colname}", header)
                        col += 1
                # ------------------------------------------------
                row += 1
                # =============================================
                ...
                for i in range(len(selected_rows)):
                    ...
                    matches = subset[mask].drop_duplicates(
                        subset=['Property Address','Owner Name/ LLC Name']
                    )
                    ...
                    for c, colname in enumerate(match_columns):
                        val = safe_excel_value(base.get(colname, ""))
